...
def search_query(filename):
    ...
    with open(filename + ".txt", "r") as f:
        ...
        subprocess.call(["BROWSER=w3m googler -n 15 " + \
                        sys.argv[1] + " " + ans[0]], shell=True, stdout=result1)
        ...
        subprocess.call(["xdotool key Return"], shell=True)
        subprocess.call(["BROWSER=w3m googler -n 15 " + \
                        sys.argv[1] + " " + ans[1]], shell=True, stdout=result2)
        subprocess.call(["BROWSER=w3m googler -n 15 " + \
                        sys.argv[1] + " " + ans[2]], shell=True, stdout=result3)
        result1.close()
        result2.close()
        result3.close()
        f.close()
